...
const MAIN_CATEGORIES = ["Casual", "Formal", "Outdoor"];
const CATEGORY_MAP: Record<string, string[]> = {
    Outdoor: ["Winterwear", "Summerwear", "Rainwear", "Springwear", "Autumnwear", "Sportswear", "Activewear"],
    Casual: ["Casual", "Streetwear", "Athleisure", "Vintage", "Minimalist", "AvantGarde", "Traditional", "Cultural"],
...
export default function FashionCatalog() {
        ...
        (prompt: string) => {
            ...
            if (MAIN_CATEGORIES.includes(prompt)) {
                ...
                if (subCategories.length > 0) {
                    router.push(`/fashion-catalog?metaCategory=${subCategories.join(",")}`);
                }
                return;
            }
        },
    ...
    const handlePromptSelect = useCallback(
        async (prompt: string) => {
            ...
            try {
                ...
                const fetchedOutfits = await outfitService.getByQuery(params.toString());
    ...
    const handleVoiceAction = useCallback(
        (action: ChatWonderAction) => {
            if (action.type === "GARMENT_RECOMMENDATION") {
               const res = action.response as { garment_data?: Record<string, unknown> } | null;
               const query = typeof res?.garment_data?.query === "string" ? res.garment_data.query : "";
               const params = new URLSearchParams(query);
               if (!params.has("limit")) params.set("limit", "4");
    ...
    useEffect(() => {
        ...
        if (!params.has("limit")) params.set("limit", "12"); // Catalog pulls a larger batch

        // Instead of handleAiComplete, directly fetch using outfitService.getByQuery
        setIsChipLoading(true);
        outfitService.getByQuery(params.toString())
            .then((fetchedOutfits) => {
                                ...
                                {Array.from({ length: totalOutfitPages }).map((_, i) => (
                                    <button
                                        ...
                                        style={{
                                            ...
                                            padding: 0,
                                            cursor: "pointer",
                                            background: i === outfitPage ? "white" : "rgba(255,255,255,0.3)",
                                            transition: "all 0.3s",
                                        }}
                                    />
                                ))}
