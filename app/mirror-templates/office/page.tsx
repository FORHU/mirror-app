...
const PREVIOUS_PICKS = [
  {
    id: "p1",
    img1: "https://images.unsplash.com/photo-1544923246-77307dd654cb?w=150&auto=format&fit=crop",
    img2: "https://images.unsplash.com/photo-1485968579580-b6d095142e6e?w=150&auto=format&fit=crop",
  },
  {
    id: "p2",
    img1: "https://images.unsplash.com/photo-1591047139829-d91aecb6caea?w=150&auto=format&fit=crop",
    img2: "https://images.unsplash.com/photo-1507679799987-c73779587ccf?w=150&auto=format&fit=crop",
  },
  {
    id: "p3",
    img1: "https://images.unsplash.com/photo-1539571696357-5a69c17a67c6?w=150&auto=format&fit=crop",
    img2: "https://images.unsplash.com/photo-1515886657613-9f3515b0c78f?w=150&auto=format&fit=crop",
  },
  {
    id: "p4",
    img1: "https://images.unsplash.com/photo-1617137968427-85924c800a22?w=150&auto=format&fit=crop",
    img2: "https://images.unsplash.com/photo-1525507119028-ed4c629a60a3?w=150&auto=format&fit=crop",
  },
];
    ...
    <main className="relative w-screen h-screen overflow-hidden bg-black flex flex-row justify-between p-6 gap-6">
      ...
      <aside className="w-[28%] h-full flex flex-col p-4 md:p-6 gap-5 bg-black/60 backdrop-blur-md border-r border-white/5 rounded-3xl z-20 overflow-y-auto scrollbar-hide">
        ...
        <div className="flex flex-col gap-3 border-b border-white/5 pb-5">
          <span className="text-xs md:text-sm text-white/40 font-bold uppercase tracking-widest px-1">
            Fashion Mode
          </span>
          <div className="flex flex-col">
            <span className="text-white text-lg md:text-xl font-extrabold tracking-wide uppercase px-1">
              Office Mode
            </span>
            <span className="text-white/40 text-xs font-bold tracking-widest px-1 mt-1">
        ...
        <div className="flex flex-col gap-3 flex-1 mt-2">
          ...
          <div className="flex flex-col gap-3.5 overflow-y-auto pr-1">
            {PREVIOUS_PICKS.map((item) => (
              <div
                key={item.id}
                className="grid grid-cols-2 gap-3 p-2.5 rounded-2xl bg-white/5 border border-white/10 hover:border-purple-400/30 transition-all cursor-pointer relative group"
              >
      ...
      <section className="flex-1 h-full flex flex-col justify-between items-center py-6 z-10 relative">
        ...
        <div className="relative z-20 flex justify-between items-start w-full px-6">
          ...
          <div className="flex flex-col items-center text-center -mt-2">
            <span className="text-white text-6xl md:text-7xl lg:text-8xl font-extralight tracking-wider">
              10:30
            </span>
            <span className="text-white/50 text-sm md:text-base font-light uppercase mt-2">
              Thursday, April 17
            </span>
          </div>
        ...
        <div className="flex flex-col items-center text-center relative z-20 max-w-sm mt-4 select-none">
          ...
          <span className="text-white/60 text-sm md:text-base font-medium mt-1.5">
            Based on today&apos;s weather and your style
          </span>
        </div>
            ...
            {OUTFIT_DETAILS.map((item, idx) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <span className="text-white text-sm font-bold block truncate -mt-0.5">
                    {item.title}
                  </span>
                  <div className="flex flex-wrap items-center gap-1.5 mt-1.5">
                    {item.bullets.map((b, bIdx) => (
                      <span
                        key={bIdx}
                        className="text-xs text-white/40 font-medium px-2.5 py-1 rounded-full bg-white/5 border border-white/5"
                      >
                        {b}
                      </span>
                    ))}
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-5 flex items-center justify-between backdrop-blur-md">
            {PALETTE.map((pal, idx) => (
              <div key={idx} className="flex flex-col items-center gap-2">
                <div
                  className={`w-10 h-10 rounded-full border border-white/20 shadow-md ${pal.color}`}
                />
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-5 backdrop-blur-md">
            <ul className="space-y-3.5">
              {REASONS.map((reason, idx) => (
                <li
                  key={idx}
                  className="flex items-start gap-2.5 text-white/80 text-xs md:text-sm font-medium leading-relaxed"
                >
