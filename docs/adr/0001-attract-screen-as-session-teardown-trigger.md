...
Arriving at `/` (the **Attract screen**) resets the chat-wonder **Voice session** — both the `sessionId` ref and the `historyRef` (last 4 turns) inside `VoiceProvider`, plus the `chatHistory` UI state. This is the _only_ state the pathname effect touches. Auth teardown, server pairing release, and gender clearing are not its responsibility.
