...
async function fetchWithCoords(lat: number, lon: number): Promise<WeatherData> {
  try {
    const r = await fetch(
      `/api/mirror/weather?lat=${lat}&lng=${lon}&_t=${Date.now()}`,
    );
    return normalizeWeather(await r.json());
  } catch {
    try {
      const r = await fetch(
        `/api/weather?lat=${lat}&lon=${lon}&_t=${Date.now()}`,
      );
      return normalizeWeather(await r.json());
    } catch {
...
async function loadWeather(): Promise<WeatherResult> {
  // Env-configured location always wins — set this on fixed kiosk deployments
  // so geolocation of the server or browser never overrides the venue.
  ...
  if (pos) {
    ...
    return {
      weather: await fetchWithCoords(pos.lat, pos.lon),
      coords: pos,
    };
  }
  ...
  return { weather: fallbackWeather(), coords: null };
}
