...
export function VoiceProvider({ children }: { children: React.ReactNode }) {
      ...
      try {
        ...
        if (pageCtxRef.current?.mode === "garment") {
          ...
          if (garmentResponse.stylist_data?.target_url) {
            await handleStylistTarget(
              garmentResponse.stylist_data.target_url,
              router,
              pathname,
            );
          }
        ...
        if (pathname.startsWith("/map")) {
          ...
          const mapLocForItinerary =
            mapStateForItinerary.userLocation ??
            mapStateForItinerary.homeLocation;
          if (
    ...
    try {
      ...
      recognition.onend = async () => {
        ...
        if (text) {
          ...
          // On the AI Assistant landing, route through the same flow as typed /
          // wake-command input (handleAIAssistantText → /api/mirror/ai-assistant
          // → [stylist]). Other screens use the cognitive pipeline.
          if (pathname.startsWith(ROUTES.AI_ASSISTANT)) {
            setTranscript(text);
            await handleAIAssistantText(text);
          } else {
  ...
  }, [voiceState, processTranscript, pathname, handleAIAssistantText]);
