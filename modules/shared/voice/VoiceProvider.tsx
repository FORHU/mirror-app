...
export function VoiceProvider({ children }: { children: React.ReactNode }) {
  ...
  const pendingMultiStopsRef = useRef<{ name: string; lat: number; lng: number; address?: string; placeId?: string; eventType?: string; timeBlock?: string }[]>([]);
  const pendingAmbiguousQueueRef = useRef<{ name: string; allResults: GeocodeResult[]; eventType?: string; timeBlock?: string }[]>([]);
  const disambiguationContextRef = useRef<{ eventType?: string; timeBlock?: string } | null>(null);
      ...
      if (pathname.startsWith("/map")) {
        ...
        // inherits stale state from the previous session.
        {
          ...
          if (ms.itineraryStops.length === 0 && ms.itineraryGroups.length === 0 && !ms.selectedDestination && !isAwaitingDisambiguationRef.current) {
            ...
            clearItineraryIdleTimer();
          } else if (ms.itineraryStops.length > 0 || ms.itineraryGroups.length > 0) {
            ...
            if (itineraryStopsRef.current.length === 0) {
              itineraryStopsRef.current = ms.itineraryStops.map((s) => ({
                name: s.name, lat: s.lat, lng: s.lng, address: s.address, placeId: s.placeId,
                eventType: s.eventType, timeBlock: s.timeBlock,
              }));
            }
          }
        }
        ...
        if (isAwaitingDisambiguationRef.current && disambiguationCandidatesRef.current.length > 0) {
          ...
          if (matched) {
            ...
            const dest = { name: matched.name, lat: matched.lat, lng: matched.lng, address: matched.address, placeId: matched.placeId, eventType: disambigCtx?.eventType, timeBlock: disambigCtx?.timeBlock };

            // If more ambiguous stops are queued, ask about the next one before plotting
            const nextAmbiguous = pendingAmbiguousQueueRef.current.length > 0
              ? pendingAmbiguousQueueRef.current.shift()!
              : null;
            if (nextAmbiguous) {
              // Accumulate resolved stop into pending so it's added when the queue empties
              pendingMultiStopsRef.current = [dest, ...pendingMultiStopsRef.current];
              disambiguationCandidatesRef.current = nextAmbiguous.allResults.slice(0, 3);
              disambiguationContextRef.current = { eventType: nextAmbiguous.eventType, timeBlock: nextAmbiguous.timeBlock };
              isAwaitingDisambiguationRef.current = true;
              const nextClarify = buildDisambiguationQuestion(nextAmbiguous.name, nextAmbiguous.allResults.slice(0, 3));
              const nextAudio = await mapService.tts(nextClarify).catch(() => null);
              setReply(nextClarify);
              historyRef.current = [...historyRef.current, { user: t, assistant: nextClarify }].slice(-4);
              setChatHistory(historyRef.current);
              setVoiceState("speaking");
              if (nextAudio) {
                const playCtx = new AudioContext(); playbackCtxRef.current = playCtx;
                const decoded = await playCtx.decodeAudioData(nextAudio.slice(0));
            ...
            if (valid.length >= 2) {
              ...
              if (ambiguous) {
                ...
                pendingAmbiguousQueueRef.current = valid
                  ...
                  .map((g) => ({ name: g.name, allResults: g.allResults, eventType: g.eventType ?? undefined, timeBlock: g.timeBlock ?? undefined }));
                // Non-ambiguous stops are ready to add once all disambiguations are resolved
                pendingMultiStopsRef.current = valid
                  .filter((g) => g !== ambiguous && !isAmbiguousGeocode(g.allResults))
                  .map((g) => ({ name: g.result.name, lat: g.result.lat, lng: g.result.lng, address: g.result.address, placeId: g.result.placeId, eventType: g.eventType ?? undefined, timeBlock: g.timeBlock ?? undefined }));
                disambiguationCandidatesRef.current = ambiguous.allResults.slice(0, 3);
                disambiguationContextRef.current = { eventType: ambiguous.eventType ?? undefined, timeBlock: ambiguous.timeBlock ?? undefined };
                isAwaitingDisambiguationRef.current = true;
        ...
        if (isItineraryPhrase(t) || isNavigationPhrase(t) || isCollectingItineraryRef.current) {
          ...
          const locationName = extractLocationFromTranscript(t)
            ...
            ?? (isCollectingItineraryRef.current
              ? t
                  .replace(/\b(yeah|yes|okay|ok|sure|also|and then|how about|add|for lunch|for dinner|for breakfast|this morning|this afternoon|this evening|tonight|this lunch|this dinner|at \d+(?::\d+)?\s*(?:am|pm)?|around \d+|please|can you|that|the)\b/gi, " ")
                  .replace(/\s{2,}/g, " ")
                  .trim() || null
              : null);
          if (locationName) {
            try {
              ...
              if (results.length > 0 && isAmbiguousGeocode(results)) {
                ...
                const displayName = locationName.split(/\s+/).length > 4 ? results[0].name : locationName;
                const clarifyReply = buildDisambiguationQuestion(displayName, results.slice(0, 3));
                const clarifyAudio = await mapService.tts(clarifyReply).catch(() => null);
                setReply(clarifyReply);
                historyRef.current = [...historyRef.current, { user: t, assistant: clarifyReply }].slice(-4);
                setChatHistory(historyRef.current);
