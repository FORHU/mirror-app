...
function inferCategoryFromName(name: string): string | undefined {
  ...
  if (/\b(church|chapel|cathedral|basilica|shrine|parish|convent|monastery|grotto)\b/.test(n)) return "restaurant";
  if (/\b(university|college|school|academy|institute|campus)\b/.test(n)) return "cafe";
  if (/\b(hospital|clinic|medical center|health center)\b/.test(n)) return "pharmacy";
  if (/\b(mall|supermarket|department store|grocery)\b/.test(n)) return "restaurant";
  if (/\b(park|garden|plaza|square|grounds)\b/.test(n)) return "cafe";
  if (/\b(market|public market|night market|tiangge|palengke)\b/.test(n)) return "restaurant";
  ...
  if (/\b(farm|eco-park|ecopark|agritourism|viewpoint|view point|lookout|vista|overlook)\b/.test(n)) return "cafe";
    ...
    stops.map(async (stop, i) => {
      try {
        const category = (stop.eventType ? EVENT_TYPE_TO_POI_CATEGORY[stop.eventType] : undefined)
          ?? inferCategoryFromName(stop.name);
